...
class Prefetch(object):
	...
	def prettyPrint(self):
		...
		print("Executable Name: {}\n".format(self.executableName.decode('ASCII')))
		...
		for i in self.volumesInformationArray:
			print(f"    Volume Name:   {i['Volume Name'  ].decode()}")
			print(f"    Creation Date: {i['Creation Date']}")
			print(f"    Serial Number: {i['Serial Number']}")
			print("")

		print("Directory Strings:")
		for volume in self.directoryStringsArray:
			for i in volume:
				print(f"    {i.decode('ASCII')}")
		print("")
		...
		for i in self.resources:
			if i:
				if count > 999:
					print("{}: {}".format(count, i.decode('ASCII')))
				if count > 99:
					print("{}:  {}".format(count, i.decode('ASCII')))
				elif count > 9:
					print("{}:   {}".format(count, i.decode('ASCII')))
...
class DecompressWin10(object):
	...
	def decompress(self, infile):
		...
		with open(infile, 'rb') as fin:
			...
			head = struct.pack('<LL', signature, decompressed_size)
			...
			# import comp as ccc
			# _, compressed = ccc.compress(algo=ccc.CompAlgo.COMPRESSION_FORMAT_XPRESS_HUFF,uncompressed=ntDecompressed)
